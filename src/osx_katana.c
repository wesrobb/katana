...
static osx_game_so_paths_t osx_get_game_so_paths()
{
        ...
        if (_NSGetExecutablePath(symbolic_exe_path, &size) == 0) {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                            "Symbolic exe path is:  %s", symbolic_exe_path);
                                        ...
                                        game_input_t *old_input)
{
        for (int i = 0; i < KATANA_MAX_CONTROLLERS; ++i) {
                ...
                game_controller_input_t *old_controller =
                    &old_input->controllers[i];
                game_controller_input_t *new_controller =
                    &new_input->controllers[i];
                ...
                osx_process_controller_button(
                    ...
                    handle, SDL_CONTROLLER_BUTTON_DPAD_DOWN);
                osx_process_controller_button(
                    &(old_controller->move_left), &(new_controller->move_left),
                    handle, SDL_CONTROLLER_BUTTON_DPAD_LEFT);
                osx_process_controller_button(&(old_controller->move_right),
                                              &(new_controller->move_right),
                                              handle,
                                              SDL_CONTROLLER_BUTTON_DPAD_RIGHT);
                osx_process_controller_button(&(old_controller->start),
                                              &(new_controller->start), handle,
                                              SDL_CONTROLLER_BUTTON_START);
                osx_process_controller_button(&(old_controller->back),
                                              &(new_controller->back), handle,
                                              SDL_CONTROLLER_BUTTON_BACK);
                osx_process_controller_button(
                    &(old_controller->left_shoulder),
                    &(new_controller->left_shoulder), handle,
                    SDL_CONTROLLER_BUTTON_LEFTSHOULDER);
                osx_process_controller_button(
                    &(old_controller->right_shoulder),
                    &(new_controller->right_shoulder), handle,
                    SDL_CONTROLLER_BUTTON_RIGHTSHOULDER);
                osx_process_controller_button(&(old_controller->action_down),
                                              &(new_controller->action_down),
                                              handle, SDL_CONTROLLER_BUTTON_A);
                osx_process_controller_button(&(old_controller->action_up),
                                              &(new_controller->action_up),
                                              handle, SDL_CONTROLLER_BUTTON_Y);
                osx_process_controller_button(&(old_controller->action_left),
                                              &(new_controller->action_left),
                                              handle, SDL_CONTROLLER_BUTTON_X);
                osx_process_controller_button(&(old_controller->action_right),
                                              &(new_controller->action_right),
                                              handle, SDL_CONTROLLER_BUTTON_B);

                i16 left_stick_x = SDL_GameControllerGetAxis(
                    handle, SDL_CONTROLLER_AXIS_LEFTX);
                i16 left_stick_y = SDL_GameControllerGetAxis(
                    handle, SDL_CONTROLLER_AXIS_LEFTY);
                i16 right_stick_x = SDL_GameControllerGetAxis(
                    handle, SDL_CONTROLLER_AXIS_RIGHTX);
                i16 right_stick_y = SDL_GameControllerGetAxis(
                    handle, SDL_CONTROLLER_AXIS_RIGHTY);
                             ...
                             game_controller_input_t *new_input)
{
        ...
        case SDLK_UP:
                ...
                break;
        case SDLK_DOWN:
                osx_process_keyup(&(old_input->move_down),
                                  &(new_input->move_down));
                break;
        case SDLK_LEFT:
                osx_process_keyup(&(old_input->move_left),
                                  &(new_input->move_left));
                break;
        case SDLK_RIGHT:
                osx_process_keyup(&(old_input->move_right),
                                  &(new_input->move_right));
                break;
        case SDLK_RETURN:
                osx_process_keyup(&(old_input->start), &(new_input->start));
                break;
        case SDLK_SPACE:
                osx_process_keyup(&(old_input->action_down),
                               ...
                               game_controller_input_t *new_input)
{
        switch (key) {
        case SDLK_UP:
                osx_process_keydown(&(old_input->move_up),
                                    &(new_input->move_up));
                break;
        case SDLK_DOWN:
                osx_process_keydown(&(old_input->move_down),
                                    &(new_input->move_down));
                break;
        case SDLK_LEFT:
                osx_process_keydown(&(old_input->move_left),
                                    &(new_input->move_left));
                break;
        case SDLK_RIGHT:
                osx_process_keydown(&(old_input->move_right),
                                    &(new_input->move_right));
                break;
        case SDLK_RETURN:
...
static game_memory_t osx_allocate_game_memory(void *base_address)
{
        ...
        memory.transient_store =
            mmap(base_address, memory.transient_store_size,
        ...
        memory.permanent_store =
            mmap((void *)(base_address + memory.transient_store_size),
                 memory.permanent_store_size, PROT_READ | PROT_WRITE,
...
{
        memcpy(dst->transient_store, src->transient_store,
               src->transient_store_size);
        memcpy(dst->permanent_store, src->permanent_store,
               src->permanent_store_size);
}
...
int main(void)
{
        ...
        game_memory_t game_memory =
            osx_allocate_game_memory((void *)Terabytes(2));
        ...
        i32 kb_controller_index = -1;

        for (i32 i = 0; i < max_joysticks; ++i) {
                if (!SDL_IsGameController(i)) {
                        if (kb_controller_index == -1) {
                                kb_controller_index = controller_index++;
                ...
                if (SDL_HapticRumbleInit(haptic_handles[controller_index]) !=
                    0) {
                        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
        ...
        game_record.input_events =
            mmap(0, sizeof(game_input_t) * KATANA_MAX_RECORDED_INPUT_EVENTS,
                 PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
        ...
        while (!quitting) {
                        ...
                        case SDL_KEYUP:
                                if (kb_controller_index != -1) {
                                        osx_handle_keyup(
                                            ...
                                            &old_input->controllers
                                                 [kb_controller_index],
                                            &new_input->controllers
                                                 [kb_controller_index]);
                                }
                                break;
                        case SDL_KEYDOWN:
                ...
                if (difftime(last_modified, game.so_last_modified) != 0) {
                        ...
                        dlclose(game.so);
                        copyfile(game_so_paths.so, game_so_paths.temp_so, 0,
                                 COPYFILE_ALL);
                        osx_load_game(&game, game_so_paths.temp_so);
                        game.so_last_modified =
                            osx_get_last_modified(game_so_paths.so);
                }
                ...
                if (recording) {
                        game_record
                            .input_events[game_record.input_record_index++] =
                            *new_input;
                } else if (playing_back) {
                        u32 playback_index =
                            game_record.input_playback_index++ %
                            (game_record.input_record_index + 1);
                        if (playback_index == 0) {
                                osx_copy_game_memory(&game_memory,
                                                     &game_record.memory);
                        }
                ...
                // texture.
                i32 texture_pitch = 0;
                SDL_LockTexture(texture, 0, (void **)&frame_buffer.pixels,
                                &texture_pitch);
                ...
                game.update_and_render_fn(&game_memory, &frame_buffer, &audio,
                                          new_input, &output, &callbacks);
                ...
                if (frame_counter++ % 180 == 0) {
                        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                                    "Frame time %.02f ms", frame_sec * 1000);
