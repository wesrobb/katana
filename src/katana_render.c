...
{
        u32 result = ((u32)(color.r * 255) & 0xFF) << 24 | ((u32)(color.b * 255) & 0xFF) << 16 |
                     ((u32)(color.g * 255) & 0xFF) << 8 | ((u32)(color.a * 255) & 0xFF);
        return result;
}
...
static void render_clear(render_cmd_clear_t *cmd, game_frame_buffer_t *frame_buffer)
{
        ...
        for (u32 y = 0; y < frame_buffer->height; ++y) {
                for (u32 x = 0; x < frame_buffer->width; ++x) {
                        *pixel = color_to_u32(cmd->color);
                        pixel++;
                }
        }
}

static void render_block(render_cmd_block_t *cmd, camera_t *cam, game_frame_buffer_t *frame_buffer)
{
        ...
        v2 frame_buffer_half_size = {frame_width_units / 2.0f, frame_height_units / 2.0f};
        ...
        if (cmd->flip_x) {
                for (i32 i = top_left_pixel.y; i < bot_right_pixel.y; ++i) {
                        ...
                        for (i32 j = top_left_pixel.x; j < bot_right_pixel.x; ++j) {
                                ...
                                linear_blend(image_data[sample_x + sample_y * cmd->image->width], dest);
                        }
                }
        } else {
                for (i32 i = top_left_pixel.y; i < bot_right_pixel.y; ++i) {
...
void render_draw_queue(render_queue_t *queue, game_frame_buffer_t *frame_buffer)
{
        for (u32 address = 0; address < queue->index;) {
                render_type_t *type = (render_type_t *)(queue->base + address);
                switch (*type) {
                case render_type_clear:
                        render_clear((render_cmd_clear_t *)type, frame_buffer);
                        address += sizeof(render_cmd_clear_t);
                        break;
                case render_type_block:
                        render_block((render_cmd_block_t *)type, queue->camera, frame_buffer);
                        address += sizeof(render_cmd_block_t);
                        break;
                case render_type_image:
                        render_image((render_cmd_image_t *)type, queue->camera, frame_buffer);
